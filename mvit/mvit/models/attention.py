...
# This file has been modified by xz4863.
...
def attention_pool(tensor, pool, hwt_shape, has_cls_embed=True, norm=None):
    if pool is None:
        return tensor, hwt_shape
    ...
    hwt_shape = [tensor.shape[2], tensor.shape[3], tensor.shape[4]]
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, hwt_shape):
        ...
        if self.pool_first:
            ...
            qkv = (
                self.qkv(x).reshape(B, N, 3, fold_dim, -1).permute(2, 0, 3, 1, 4)
            )
        ...
        # positional embedding temporarily disabled
        # if self.rel_pos_spatial:
        ...
        x = x.transpose(1, 2).reshape(-1, N, self.dim_out)
        x = self.proj(x)
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        if len(stride_q) > 0 and numpy.prod(stride_q) > 1:
            ...
            self.pool_skip = nn.MaxPool3d(kernel_skip, stride_skip, padding_skip, ceil_mode=False)
        else:
            self.pool_skip = None

    def forward(self, x, hwt_shape):
        x_norm = self.norm1(x)
        x_block, hwt_shape_new = self.attn(x_norm, hwt_shape)

        if self.dim_mul_in_att and self.dim != self.dim_out:
            x = self.proj(x_norm)
        x_res, _ = attention_pool(
            x, self.pool_skip, hwt_shape, has_cls_embed=self.has_cls_embed
        )

...
# This file has been modified by xz4863 (Xu Zhang).
# Original work by the Meta Platforms under the Apache License 2.0.
...
class MViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.PATCH_KERNEL = cfg.MVIT.PATCH_KERNEL
        self.PATCH_STRIDE = cfg.MVIT.PATCH_STRIDE
        self.PATCH_PADDING = cfg.MVIT.PATCH_PADDING
    ...
    def forward(self, x):
        # if the input dimension is (B, C, H, W) i.e. batch_size, input_channel, height, width
        # add an additional depth dimension to represent frames of videos
        if (x.dim() == 4):
            x = x.unsqueeze(2)
        # The depth after patch embedding. This equivalent to bchw[-3] after patch embedding.
        T = int((x.shape[2] + (2 * self.PATCH_PADDING[0] ) - self.PATCH_KERNEL[0]) // self.PATCH_STRIDE[0] + 1)

        x, bchw = self.patch_embed(x)

        # Number of Frame is T computed previously.
        H, W = bchw[-2], bchw[-1]

        # B is batch size, N is dimension for the embedded representation of video/image (HWT all concat together)
        # C is out channel number
        B, N, C = x.shape
        ...
        # blk contains all the transformer blocks
        # apply the transformer blocks sequentially

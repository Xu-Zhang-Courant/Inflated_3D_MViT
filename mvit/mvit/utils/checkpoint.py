...
# This file has been modified by Xu Zhang (xz4863).
...
def load_checkpoint(
    ...
    # if instead want to load 2D weights, enable squeeze_temporal option and run this
    # if squeeze_temporal:
    ...
    # if the size and names of checkpoints' weights match with those of the current model, load them
    # if the size does not match (the weights is from a 2D model), don't load positional embedding weights and
    # create an additional depth dimension for video frames in the layer is a Conv/transformer layer
    # additional scaling is considered to stablize variances across the depth dimension
    for k, v in pre_train_dict.items():
        if (v.shape == (model_dict[k]).shape):
            pre_train_dict_match[k] = v
        else:
            if (('attn.rel_pos_w' in k) or ('attn.rel_pos_h' in k)):
                pass
            else:
                pre_train_dict_match[k] =(v.unsqueeze(2))/ (model_dict[k].shape[2] ** (0.5))
